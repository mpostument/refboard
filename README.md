...
- **Browse your library** - the app is three columns: your packs and their
  ...
  In the grid, the search box matches file, folder and pack names, any
  thumbnail opens full-screen in the same viewer a session uses (zoom, angle
  tool, construction guides, tonal-value split, eyedropper - with `← →`
  walking that folder in order and no clock running), and **Draw these**
  starts a real timed session from exactly what is on screen, search filter
  included. **Skip** in a thumbnail's corner marks a pose never to draw
  again; skipped images stay in the grid, dimmed, so it can be undone.
